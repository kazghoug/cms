            ...
            <div id="content">
                ...
                <div>
                    <form class="mx-3" method="POST" enctype="multipart/form-data">
                        ...
                        <div id="editor" class="form-control mb-2"></div>
                        <label for="" class="mb-2">Categorie</label>
                        <Select class="d-block mb-2" name="categorie">
                            <?php 
                        ...
                        <Select class="d-block mb-2" name="author">
                            <?php 
...
<?php
  ...
  if (isset($_POST["submit"])) {
    ...
    try {
        $sql = "SELECT 	id FROM auteur WHERE name='".$author."'";
        $stmt = $conn->prepare($sql);
        $stmt->execute();
        $result = $stmt->setFetchMode(PDO::FETCH_ASSOC);
        foreach(new TableRows(new RecursiveArrayIterator($stmt->fetchAll())) as $k=>$v) {
        ...
        echo "1st catch Connection failed: " . $e->getMessage();
    }
    try {
        $sql = "SELECT 	id FROM categorie WHERE name='".$categorie."'";
        $stmt = $conn->prepare($sql);
        $stmt->execute();
        $result = $stmt->setFetchMode(PDO::FETCH_ASSOC);
        foreach(new TableRows(new RecursiveArrayIterator($stmt->fetchAll())) as $k=>$v) {
        ...
        $id_author= $_SESSION["idauthor"];
      $sql = "INSERT INTO article(title, text, image,id_categorie,id_auteur) VALUES ('$title' , 'hgonrog', '$filename',$id_categorie,$id_author)";
